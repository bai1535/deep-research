...
## 2026-09-03: MCTS-RAG 2.0 纯算法脚手架 F35
- 原因: 2.0 需要在高难多跳问题上把“检索 × 推理”变成树搜索，而不是固定流水线
- 方案: 先建立纯算法层 `deep_research/mcts_rag`，包含 A1–A6 动作空间、
  MCTSNode/UCT/backpropagate、候选答案归一化分组投票；不接 LLM/工具
- 否决方案: 一开始就实现完整 MCTS-RAG runner（依赖 LLM/搜索，难以单独测试）
- 范围: 纯算法脚手架；下一步做 LLM 动作执行器与 BrowseComp 实验入口
